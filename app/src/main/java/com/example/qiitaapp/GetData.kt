...
class GetData<T> constructor(listView: ListView, activity: T, scope: String, status: TextView): AsyncTask<Int, Int, MutableList<Map<String, String>>>() where T : AppCompatActivity{
    val weakActivity = WeakReference(activity)
    private val activity:Activity = activity
    private val listView: ListView = listView
    private val status: TextView = status

    private val scope:String = scope
    var detailLists = mutableListOf<Map<String, String>>()
    ...
    override fun doInBackground(vararg p0: Int?): MutableList<Map<String, String>> {
        ...
        detailLists.add(mapOf("title" to "タイトル", "author" to "筆者", "url" to ""))
            ...
            val document = Jsoup.connect("https://qiita.com/?scope=$scope").get()
            ...
            for (ed in edge) {
                var url = baseUrl + "/" + ed.node.author.urlName + "/items/" + ed.node.uuid
                var map = mapOf(
    ...
    override fun onPostExecute(result: MutableList<Map<String, String>>) {
        ...
        var adapter: SimpleAdapter = SimpleAdapter(this.activity, result, R.layout.listcontent,
